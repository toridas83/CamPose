...
class PostureService:
    ...
    def start_baseline_capture(self, seconds: float = 10.0) -> None:
        self.ensure_camera()
        self._baseline_samples = []
        self._baseline_deadline = time.monotonic() + seconds
        with self._lock:
    ...
    def _run(self) -> None:
        ...
        try:
            ...
            feature_window: deque[dict[str, float]] = deque(maxlen=5)
    ...
    def _handle_baseline(self, features: dict[str, float] | None, frame_shape: tuple[int, ...]) -> None:
        if self._baseline_deadline <= 0.0:
            return
        remaining = self._baseline_deadline - time.monotonic()
