...
class BaselinePage(ctk.CTkFrame):
    ...
    def _build_ui(self) -> None:
        ...
        ctk.CTkLabel(
            self,
            text=(
                "시작 후 3초 동안 자세를 준비한 뒤, 바른 작업 자세를 10초간 측정합니다.\n"
                "정면을 보고 상체가 화면 중앙에 오도록 앉아 주세요. "
                "화면이 보이지 않으면 설정에서 사용할 카메라를 선택해 주세요."
            ),
            font=FONT_BODY,
        ...
        self.capture_button = ctk.CTkButton(
            buttons, text="3초 준비 후 측정", font=FONT_BUTTON, fg_color=COLOR_ACCENT,
    ...
    def _start_capture(self) -> None:
        ...
        if not snapshot.person_detected:
            messagebox.showwarning(
                "사람 미감지",
                "먼저 카메라에 상체가 보이도록 앉아 주세요.\n영상이 보이지 않으면 설정에서 사용할 카메라를 선택해 주세요.",
                parent=self,
            )
            return
        self.service.start_baseline_capture(10.0, prepare_seconds=3.0)
    ...
    def _poll(self) -> None:
        ...
        if snapshot.baseline_capturing:
            if snapshot.baseline_preparing:
                self.progress.set(0)
                self.status_label.configure(
                    text=f"{snapshot.baseline_prepare_remaining:0.1f}초 후 측정을 시작합니다 · 바른 자세를 준비해 주세요",
...
class DeveloperCameraPage(ctk.CTkFrame):
    ...
    def _build_ui(self) -> None:
        ...
        ctk.CTkLabel(header, text="자세 분석 화면 · 실시간 측정값", font=FONT_HEADING, text_color=COLOR_TEXT).grid(
    ...
    def _poll(self) -> None:
        ...
        for title, key in self.METRICS:
            ...
            if current is None or base is None:
                ...
                delta = line_angle_delta(current, base) if key in {"eye_angle", "shoulder_angle"} else current - base
                value = f"{current:+.3f} / {base:+.3f} / {delta:+.3f}"
...
class MonitoringPage(ctk.CTkFrame):
    ...
    def _build_header(self) -> None:
        ...
        ctk.CTkLabel(
            header, text="영상은 저장하지 않으며, 기준 자세 설정과 자세 분석 화면에서만 카메라 영상을 표시합니다.",
    ...
    def _build_overview(self) -> None:
        ...
        ctk.CTkButton(
            card, text="자세 분석 화면 열기", font=FONT_BUTTON, height=42,
